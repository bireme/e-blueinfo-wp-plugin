...
<div class="row-fluid">
    <h2 class="h2-loop-tit">
        <a href="<?php echo real_site_url($memoria_azul_plugin_slug); ?>resource/?id=<?php echo $resource->id; ?>">
...
<?php if ($resource->scope_region): ?>
    <div class="row-fluid">
        <?php _e('Act country/region','memoria-azul'); ?>: <strong><?php print_lang_value($resource->scope_region, $lang) ;?></strong>
    </div>
<?php endif; ?>

<?php if ($resource->organ_issuer): ?>
    <div class="row-fluid">
        <?php _e('Organ issuer','memoria-azul'); ?>: <strong><?php print_lang_value($resource->organ_issuer[0], $lang);?></strong>
    </div>
<?php endif; ?>

<?php if ($resource->scope && $detail_page) : ?>
    <div class="row-fluid">
        <?php _e('Act scope','memoria-azul'); ?>:
        <strong>
...
<?php if ($resource->source_name && $detail_page): ?>
    <div class="row-fluid">
        <?php _e('Source','memoria-azul'); ?>: <strong><?php print_lang_value($resource->source_name, $lang) ;?></strong>
    </div>
<?php endif; ?>
...
<?php if ($resource->publication_date && $detail_page) : ?>
    <div class="row-fluid">
        <?php _e('Publication date','memoria-azul'); ?>:
    ...
    <?php foreach ( $resource->relationship_active as $rel) { ?>
        <div class="row-fluid">
            ...
            <?php
                ...
                if ($rel_act_link != ''){
                    echo '<a href="' . real_site_url($memoria_azul_plugin_slug) . 'resource/?id=' . $rel_act_link . '">';
                }
                echo '&nbsp';
    ...
    <?php foreach ( $resource->relationship_passive as $rel) { ?>
        <div class="row-fluid">
            ...
            <?php
                ...
                print_lang_value($rel_act_type, $lang);
                echo '&nbsp';
                echo $rel_act_number . ', ' . __('of', 'memoria-azul'). '&nbsp';
                echo format_act_date($rel_act_date, $lang) . '&nbsp';
                if ($rel_act_link != ''){
                    echo '</a>';
                }
    ...
    <div id="conteudo-loop-tags" class="row-fluid margintop10">
        ...
        <?php
            ...
            foreach ( $resource->descriptor as $index => $subject ):
                echo "<a href='" . real_site_url($memoria_azul_plugin_slug) . "?q=descriptor:\"" . $subject . "\"'>" . $subject . "</a>";
        ...
        <?php foreach ( $resource->fulltext as $fulltext) { ?>
            ...
            <span class="more">
                <a href="<?php echo $document_url ?>" target="_blank">
                    <?php _e('Text in','memoria-azul') ?> <?php echo $fulltext_lang[$document_lang] ?>
                </a>
            </span>&nbsp;&nbsp;
        <?php } ?>
    </div>
